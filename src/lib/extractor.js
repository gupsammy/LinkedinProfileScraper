...
function extractProfilesFromPage() {
  try {
    const profiles = [];
    const { resultSelectors, profileLinkSelectors, nameSelectors } = window.LinkedInScraperSelectors || {};
...
function extractSingleProfile(result, index) {
  try {
    const { profileLinkSelectors, nameSelectors } = window.LinkedInScraperSelectors || {};
  ...
  for (const nameSelector of nameSelectors) {
    ...
    if (nameElement) {
      ...
      if (extractedName && isValidName(extractedName)) {
        console.log(`Found name using selector "${nameSelector}": "${extractedName}"`);
...
function isValidName(name) {
  ...
  return name.length > 1 &&
    name.length < 100 &&
    !isStatusText &&
    /^[a-zA-Z\s\u00C0-\u017F\u0100-\u024F\u1E00-\u1EFF\u2000-\u206F\u0400-\u04FF\u0500-\u052F\u2DE0-\u2DFF\uA640-\uA69F\u0370-\u03FF\u1F00-\u1FFF()]+/.test(name);
}
